...
def page_records():
    ...
    with download_col:
        ...
        # Serve the already-generated Excel file when it is current. The file is
        # regenerated by update_excel_file() after every save, so rebuilding it
        # on every rerun (as the old code did) was pure waste. If the DB is
        # newer than the Excel file (e.g. after a restore/import), rebuild once.
        # On PostgreSQL/cloud the DB mtime is not a reliable change signal, so
        # keep the old conservative rebuild there.
        excel_ok = False
    ...
    with sc4:
        ...
        # Lightweight COUNT-only query instead of scanning rows page_size=1
        total_count = db.count_search_records(
