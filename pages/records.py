...
def page_records():
    ...
    with download_col:
        ...
        # Excel export is built LAZILY and served on request. Record/invoice
        # saves never rebuild the workbook (no eager export in any save path).
        # The workbook is produced only when the user clicks to download it and
        # is then remembered for this session until the DB fingerprint changes.
        excel_fp = db.get_db_fingerprint()
        excel_ready = (st.session_state.get("_excel_fp") == excel_fp
                       and st.session_state.get("_excel_bytes") is not None)
        if excel_ready:
            st.download_button("⬇️ Download Excel File",
                               st.session_state["_excel_bytes"],
                               file_name="ALL_RECORDS.xlsx",
                               key="excel_download",
                               use_container_width=True)
        else:
            if st.button("⬇️ Download Excel File", key="excel_prepare",
                         use_container_width=True):
                with st.spinner("Preparing Excel workbook..."):
                    st.session_state["_excel_bytes"] = _excel_download_bytes(excel_fp)
                st.session_state["_excel_fp"] = excel_fp
                st.rerun()
